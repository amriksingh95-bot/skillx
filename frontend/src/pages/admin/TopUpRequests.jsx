...
export default function TopUpRequests() {
  ...
  const fetchPending = async () => {
    setLoading(true);
    try {
      const res = await api.get('/api/admin/topup/pending');
  ...
  const handleConfirm = async (topUpId) => {
    setActioning(topUpId);
    try {
      const res = await api.patch(`/api/admin/topup/${topUpId}/confirm`);
      toast.success(res.data.message);
      await fetchPending();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to confirm top-up.');
    } finally {
      setActioning(null);
    }
  };
  ...
  const handleReject = async (topUpId) => {
    setActioning(topUpId);
    try {
      const res = await api.patch(`/api/admin/topup/${topUpId}/reject`);
      toast.success(res.data.message);
      await fetchPending();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to reject top-up.');
    } finally {
      setActioning(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-white">Top-Up Requests</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review and process merchant point top-ups</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl p-12 text-center">
          ...
          <p className="text-sm font-medium text-slate-500">No pending top-up requests.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-dark-border">
                {topUps.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300 capitalize">{item.packageName}</td>
